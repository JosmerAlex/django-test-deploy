...
var traslado = {
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                       // return row.estado.name
                        ...
                        if(row.estado == 'APR') return '<i class="fas fa-check-circle" data-toggle="tooltip" title="Aprobado" style="color:green"><span style="display:none;">'+row.estado.name+'</span></i>';
                ...
                {            
                    ...
                    render: function (data, type, row) {
                        if(row.estado == 'APR'){                           
                            var buttons = '<a rel="VerDetTraslado" class="btn btn-success btn-xs" data-toggle="tooltip" title="Consultar Detalle"><i class="fas fa-search"></i></a> ';
                            ...
                            var buttons = '<a href="/erp/traslado/update/' + row.id + '/" class="btn btn-warning btn-xs" data-toggle="tooltip" title="Editar"><i class="fas fa-edit" style="color: #ffffff;"></i></a> ';
                            buttons += '<a href="#" rel="delete" class="btn btn-danger btn-xs" data-toggle="tooltip" title="Eliminar"><i class="fas fa-trash-alt"></i></a> ';
                            buttons += '<a rel="VerDetTraslado" class="btn btn-success btn-xs" data-toggle="tooltip" title="Consultar Detalle"><i class="fas fa-search"></i></a> ';
                            return buttons;    
                        }                        
                        }
                    },
    ...
    formatRowHtml: function (data) {
        ...
        html += '<thead class="thead-dark upper-text">';
        ...
        $.each(data, function (key, value) {
            ...
            html+='<td>'+value.nombredepar_dest+'</td>'
...
$(function () {
    ...
    $('.btnSearchAll').on('click', function () {
      ...
      $('#data_list tbody').on('click', 'a[rel="delete"]', function () {
        ...
        url = "/erp/traslado/delete/"+data.id+"/";
        submit_with_ajax(url, 'Notificación', '¿Estas seguro de eliminar el Traslado?  ' + '<b style="color: #304ffe;">' + data.cod_traslado + '</b>', parameters, function () {
            tblTraslado.ajax.reload();
        });
      });

      $('#data_list tbody')
        .on('click', 'a[rel="VerDetTraslado"]', function () {
            ...
            $('#title_detail_tras').find('span').html('Consulta Detalle del Traslado:  ' + '<b style="color: #ffffff;">' + data.cod_traslado + '</b>');
            $('#myModalDetProdTraslado').modal('show');
        })
        .on('click', 'td.details-control', function () {
            var tr = $(this).closest('tr');
            var row =  tblTraslado.row(tr);
            var id=row.data().id;
