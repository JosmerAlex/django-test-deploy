        ...
        columnDefs: [
            {
                ...
                render: function (data, type, row) {
                    ...
                    if(row.estado.id == 'SAS')return '<span class="badge rounded-pill badge-success" style="font-size: 10px;">'+data+'</span>'
                    if(row.estado.id == 'ANU')return '<span class="badge rounded-pill badge-danger" style="font-size: 10px;">'+data+'</span>'
                }
            },
            {
                targets: [-1],
...
$('#frmCodbienes').on('submit', function (e) {
    ...
    if  ($('input[name="action"]').val() == 'add'){
        ...
        url = "/erp/codbien/update/"+param_id+"/";
...
$('#data_list tbody').on('click', 'a[rel="edit"]', function () {
    //$('#productos tbody').on('click', '.nombreclase', function () {
      ...
      $('#idunidad_Cen').val(data.procedencia).trigger('change.select2');
      $('#idunidad_Loc').val(data.unidad_admin).trigger('change.select2');
