...
class Unidad(models.Model):
    ...
    tipo_unidad= models.CharField(max_length=3, choices=tipounidad_choices, null=True, blank=True, verbose_name='Tipo de Unidad')
...
class Almacen(models.Model):
    ...
    ced=models.CharField(max_length=10, null=True, blank=True, verbose_name='Cedula')
...
class GrupoCtaBienes(models.Model):
...
class SubGrupoCtaBienes(models.Model):
...
class Producto(models.Model):
    ...
    tipo_item=models.CharField(max_length=2, choices=tipo_item_choice, default='P', verbose_name='Tipo de Item')
    imagen=models.ImageField(upload_to='producto', default='producto/img_articulo.png', null=True, blank=True, verbose_name='Insertar Imagen')
    created=models.DateTimeField(auto_now_add=True, verbose_name='Creación')
    updated=models.DateTimeField(auto_now_add=True, verbose_name='Ultima modificación')
 ...
 #   __AUTOCODE__ = 'COM'
    ...
    def get_imagen(self):
        ...
        return '{}{}'.format(STATIC_URL, 'producto/img_articulo.png')

    def toJSON(self):
        item = model_to_dict(self)
        item['full_name'] = '{} / {}'.format(self.nombre, self.categorias.nombre)
        item['tipo_item'] = {'id': self.tipo_item, 'name': self.get_tipo_item_display()}
        ...
        item['grupobien'] = self.grupobien.toJSON()
...
class ControlStock(models.Model):
    productos=models.ForeignKey(Producto, on_delete=models.CASCADE, null=True, blank=True, related_name='Catproducto_set')
    almacenes=models.ForeignKey(Almacen, default=1, on_delete=models.CASCADE, blank=True, null=True, related_name='prod_almacen_set', verbose_name='Almacen')
    stock_actual=models.PositiveIntegerField(default=0, null=True, blank=True, verbose_name='Stock Actual')
    precio=models.DecimalField(max_digits=14, decimal_places=2, default=0.00, null=True, blank=True, verbose_name="Precio")
    stock_min=models.PositiveIntegerField(default=0, null=True, blank=True, verbose_name='Stock Minimo')
    stock_max=models.PositiveIntegerField(default=0, null=True, blank=True, verbose_name='Stock Maximo')  
...
class Proveedor(models.Model):
    ...
    ced_repre=models.CharField(max_length=10, unique=True, null=True, blank=True, verbose_name='Cedula representante')
...
class CodBienes(models.Model):
    codbien=models.CharField(max_length=80, unique=True, verbose_name='Código de Bien')
    procedencia=models.ForeignKey(Unidad, on_delete=models.CASCADE, blank=True, null=True, related_name='codbienes_unidacental_set', verbose_name='Unidad Admin. Central')
    unidad_admin=models.ForeignKey(Unidad,on_delete=models.CASCADE, blank=True, null=True, related_name='codbienes_unidaadmin_set', verbose_name='Unidad Admin. Local')
    estado= models.CharField(max_length=3, choices=estadoCodigobien_choices, default='SAS', null=True, blank=True, verbose_name='Estado Disponibilidad')
    created=models.DateTimeField(auto_now_add=True, verbose_name='Creación')
    updated=models.DateTimeField(auto_now_add=True, verbose_name='Ultima modificación')
...
class ConcepMovimiento(models.Model):
...
class SalidaProduc(models.Model):
    cod_salida= models.CharField(max_length=14, blank=True, null=True, verbose_name='Código distribución')
    origen = models.ForeignKey(Almacen, on_delete=models.CASCADE, blank=True, null=True, related_name='salidaproduc_almacen_set', verbose_name='Origen distribución')
    respon_origen=models.CharField(max_length=120, null=True, blank=True, verbose_name='Responsable Origen')
    destino = models.ForeignKey(Unidad, on_delete=models.CASCADE, blank=True, null=True, related_name='salidaproduc_destino_set', verbose_name='Destino distribución')
    respon_destino=models.CharField(max_length=120, null=True, blank=True, verbose_name='Responsable Destino')
    tipo_salida= models.ForeignKey(ConcepMovimiento, on_delete=models.CASCADE, verbose_name='Tipo de Salida')
    tipo_comprob = models.CharField(max_length=3, null=True, blank=True, choices=tipocomprobante_choices, verbose_name='Tipo comprobante')
    num_comprob= models.CharField(max_length=24, null=True, blank=True, verbose_name='Numero Comprobante')
    ...
    fecha_salida = models.DateField(default=datetime.now, verbose_name='Fecha distribución')
    usuario=models.ForeignKey(AUTH_USER_MODEL, on_delete=models.CASCADE, blank=True, null=True, related_name='salidaproduc_user_set', verbose_name='Usuario')
    observ=models.CharField(max_length=200, null=True, blank=True, verbose_name='Observación')
    estado= models.CharField(max_length=3, choices=estadoDocumento_choices, default='DIS', verbose_name='Estado distribución')
...
class IngresoProduc(models.Model):
    ...
    observ=models.CharField(max_length=200, null=True, blank=True, verbose_name='Observación')
    estado= models.CharField(max_length=3, choices=estadoDocumento_choices, default='DIS', verbose_name='Estado ingreso')

    def __str__(self):
        return self.proveedor.empresa
    ...
    def toJSON(self):
        ...
        return item
    #NOTA: en estos metodos para crear diccionarion podemos crear un campo adicional y extraer los registros relacionados en otras tablas 
...
class TrasladoProduc(models.Model):
    ...
    estado= models.CharField(max_length=3, choices=estadoDocumento_choices, default='DIS', verbose_name='Estado Operación')
   ...
   # objects = SaleManager()
    ...
    # def delete(self, using=None, keep_parents=False):
    #     for det in self.dettrasprod_set.all():
    #         det.prod.stock_actual += det.cant
    #         det.prod.save()
    #     super(TrasladoProduc, self).delete()
...
class DesincProduc(models.Model):
    ...
    observ=models.CharField(max_length=200, null=True, blank=True, verbose_name='Observación')
    estado= models.CharField(max_length=3, choices=estadoDocumento_choices, default='DIS', verbose_name='Estado desincorporación')
...
class InventarioBienes(models.Model):
    ...
    ult_proc=models.CharField(max_length=6, null=True, blank=True, verbose_name='Ultimo procedimiento')
    ant_ult_proc=models.CharField(max_length=6, null=True, blank=True, verbose_name='Ultimo procedimiento anterior')
    ...
    ant_tipo_proc = models.ForeignKey(ConcepMovimiento, on_delete=models.CASCADE, null=True, blank=True, related_name='tipo_proc_ant_invbienes_set', verbose_name='Tipo de entrada anterior')
...
class Empresa(models.Model):
    ...
    multialmacen=models.BooleanField(default=False, null=True, blank=True, verbose_name='Multi almacen')
    created=models.DateTimeField(auto_now_add=True, verbose_name='Creación')
    updated=models.DateTimeField(auto_now_add=True, verbose_name='Última actualización')
    estado=models.BooleanField(default=True, max_length=4, null=True, blank=True, verbose_name='Estado')
